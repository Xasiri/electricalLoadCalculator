...
export const cableCalculationHandler = (array)=>{
  let numberofCables = array.map(i => i.quantity).reduce((a,c) => a+c,0);
  let totalCableOuterDiameter = array.map(i => i.diameter*i.quantity).reduce((a,c) => a+c,0);
  let widthofAllCables = numberofCables*10 + totalCableOuterDiameter;
  let  finalWidthofAllCables= 1.2*widthofAllCables.toFixed(2);
  let weightofAllCables = array.map(i => i.quantity*i.density/1000).reduce((a,c) => a+c,0).toFixed(2);
